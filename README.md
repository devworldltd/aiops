...
> 스킬 35개·에이전트 26개의 목록과 결과물, 마커 헤더 인터페이스, 게이트 통과 조건,
...
- **단일 플러그인 `aiops`** — 스킬 35 + 에이전트 26 + 템플릿(E2E·모바일)을 하나의 네임스페이스 `aiops:`에.
  - 스킬 35 중 **3개(`backend`·`frontend`·`wireframe`)는 `disable-model-invocation: true`** 로
...
│   ├── skills/     (35)                  # devflow·bugflow·chainflow·setup·qa-*·merge-*·focus·unfocus·explain-app …
│   ├── agents/     (26)                  # planning·dev·dev-backend·qa-e2e·dev-pr …
