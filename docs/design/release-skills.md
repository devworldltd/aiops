...
> **`org` 항목의 legacy 폴백은 이관 유예이지 대체 경로가 아니다.**
> 2순위는 `<접두사><종류>@<현재 앱 슬러그>` 이므로 **사본을 가진 그 앱 하나만** 구제한다.
> `pong` 은 `PONG_APPSTORE_API_KEY_JSON@pong` 으로 잡히지만, `zen-koi` 가 찾는
> `ZEN_KOI_APPSTORE_API_KEY_JSON@zen-koi` 는 **영원히 존재하지 않는다.**
>
> 이 동작 자체는 맞다 — 한 앱이 다른 앱의 네임스페이스를 뒤지게 만들면 권한 경계를 넘는다.
> 바뀌는 것은 결론이다: **이관은 선택이 아니라 두 번째 앱 출시의 선행 조건이다.**
> 앱이 하나 늘 때마다 `not_found` 로 막히는 앱이 하나씩 늘어난다.

### 1순위·2순위가 둘 다 있으면 조용히 고르지 않는다
...
| 지문이 다르다 | `duplicate_conflict` 로 **중단**. `HANDOFF_DECISION` 을 남기고 추측하지 않는다 |

지문은 값의 SHA-256 앞 8자다. **값을 노출하지 않고 같은지만 비교**하므로 보고에 남겨도 안전하다.

충돌이 `REQUIRED` 가 아니라 `DECISION` 인 이유는, 스킬이 **선택지를 제시할 수 있기** 때문이다 —
어느 이름에 어떤 지문이 있는지 보여줄 수 있다. 계약 v1 의 분리 기준 그대로다.
