...
check "$(grep -q '되돌릴 수 없는 것은 둘이다' "$SKILL" && echo 1)" "F1 불가역 항목이 둘임을 명시한다"

# zen-koi 첫 실사용(2026-09-20)에서 다섯 번째가 나왔다 — 무료→유료 불가.
# **"넷 중 하나" 로 적었던 것이 틀렸다.** 표에 그 행이 있는지, 그리고 복구 경로가
# 다른 불가역 항목을 건드린다는 것까지 적혀 있는지 본다.
check "$(grep -q '무료로 제공한 후에는 유료로 변경할 수 없습니다' "$SKILL" && echo 1)" \
      "F1b 무료→유료 불가를 원문으로 인용한다"
check "$(grep -q '새 패키지 이름으로 새 앱' "$SKILL" && echo 1)" \
      "F1c 복구 경로가 패키지명을 다시 건드린다는 것을 적는다"
check "$(grep -q '유료 → 무료 | 가능' "$SKILL" && echo 1)" \
      "F1d 반대 방향은 가능하다고 구별한다 (양방향 불가로 뭉개지 않는다)"

# 확정 시점에 대해 근거 둘이 엇갈린다 — 단정하지 않되 보수적으로 다뤄야 한다
check "$(grep -q '근거 둘이 엇갈린다' "$SKILL" && echo 1)" "F1e 확정 시점의 근거 충돌을 숨기지 않는다"
check "$(grep -q '지금 이 화면에서 고정됩니다' "$SKILL" && echo 1)" \
      "F1f 확인 화면이 '지금 고정된다' 고 말한다 ('나중에' 로 읽힐 여지를 없앤다)"
# 경고를 전부에 달면 구별이 사라진다 — 두 줄에만 달렸는가
WARN=$(sed -n '/앱 이름     Zen Koi/,/이대로 만들까요/p' "$SKILL" | grep -c '←.*불가')
check "$([[ "$WARN" == "2" ]] && echo 1)" "F1g 확인 화면의 불가역 경고가 정확히 두 줄 (실측: ${WARN})"
