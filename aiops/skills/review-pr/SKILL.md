...
> ⚠️ 자기 PR REQUEST_CHANGES(Gitea): Gitea 는 자기 PR 에 대해 **APPROVE 뿐 아니라 REQUEST_CHANGES 도 거부**한다. `forge.sh` 는 이 경우에도 COMMENT 로 강등하고 경고를 출력한다(이슈 #65 이전에는 강등 매칭이 approve 계열 문구만 봐서 리뷰가 조용히 버려졌다).
>
> 강등되면 Gitea 상태는 `COMMENT` 지만 **판정은 리뷰 본문의 「결론」이 정본**이다. 자기 PR 에 REQUEST_CHANGES 를 낸 경우, 상태만 보고 통과로 오해하지 않도록 PR 에 판정을 명시하는 코멘트를 따로 남긴다.

> ⚠️ 종료 코드: `forge.sh pr-review` 는 등록에 실패하면(응답에 `id` 없음) 사유를 stderr 로 출력하고 **4** 로 끝난다. `REVIEW_ID=` 가 비어 있는데 0 으로 끝나던 예전 동작에 기대어 성공을 판정하지 말 것 — 호출 후 종료 코드를 확인한다.
