...
| **무료 → 유료** | **불가** | Play: "앱을 무료로 제공한 후에는 유료로 변경할 수 없습니다" |
| 유료 → 무료 | 가능 | Play: "앱을 유료에서 무료로 변경할 수 있습니다" |
| 기본 언어 | 가능 | Play: "이는 나중에 변경할 수 있습니다" |
| 스토어 앱 이름 | 가능 | 위와 같은 문장 |
| 앱/게임 구분 | 가능 | Play: "이는 나중에 변경할 수 있습니다" |

**되돌릴 수 없는 것은 둘이다 — 패키지명과 "무료로 낸다" 는 결정이다.** 나머지에 같은 무게를 실으면
정작 중요한 두 줄이 묻힌다.

### 무료→유료는 사실상 패키지명과 같은 제약이다

가격 방향은 한쪽으로만 흐른다. 되돌리는 경로가 있긴 한데, 그게 다른 불가역 항목을 다시 건드린다.

> 앱을 유료로 변경하려면 **새 패키지 이름으로 새 앱을 만들고** 가격을 설정해야 합니다.

즉 무료로 내는 순간 **그 패키지명으로는 영원히 유료가 될 수 없다.** 기존 설치·리뷰·순위는 이어지지
않는다. 두 항목이 사실은 하나로 묶여 있다 — 확인 화면에서 함께 보여준다(§5).

> **처음에 "넷 중 하나만 불가역" 이라고 적었는데 다섯 번째가 있었다.** zen-koi 가 첫 실사용에서
> 콘솔 화면에 적힌 문구를 보고 알렸다(2026-09-20). 공식 문서로 확인한 결과 그쪽 보고보다
> **더 강했다** — "게시 후" 가 아니라 **"무료로 제공한 후"** 이고, 복구 경로가 새 패키지명을
> 요구한다.

> iOS 는 생성 **이후**가 다르다. ASC API `Modify an app` 이 **번들 ID 와 기본 언어까지** 고칠 수
> 있다. Android 패키지명은 영구다. **비대칭은 생성이 아니라 생성 이후에 있다.**

### 불가역 지점은 **이 스킬 안에** 있다고 보고 다룬다

패키지명이 확정되는 시점에 대해 **근거 둘이 엇갈린다.**

```
Play Console 고객센터   '앱 만들기' 절차에 패키지명 입력 단계가 없다
                        → 첫 AAB 업로드에서 확정되는 것으로 읽힌다
zen-koi 실측 (09-20)    '앱 만들기' 폼이 패키지명을 직접 입력받았고
                        '사용 가능 여부 확인' 링크로 중복 검사까지 했다
```

**어느 쪽인지 단정하지 않는다.** 고객센터 문서가 UI 변경을 따라오지 못했을 수 있고, 계정·지역·
콘솔 버전에 따라 화면이 다를 수도 있다. 둘 다 확인된 것은 아니다.

**다만 보수적인 쪽으로 다룬다 — 이 스킬 안에서 확정된다고 가정한다.** 두 가정 중 이쪽이 틀렸을 때의
손해가 작다.

```
여기서 확정된다고 가정했는데 실제로는 나중  경고를 한 번 더 본 것뿐이다
나중이라고 가정했는데 실제로는 여기서 확정  "아직 되돌릴 수 있겠지" 로 읽고 지나간다
```

§5 문구는 **"지금 이 화면에서 고정된다"** 로 쓴다. "나중에 업로드할 때 고정된다" 고 쓰지 않는다.

## §5 확인 — 한 화면에 모아 승인받는다

```
앱 이름     Zen Koi
패키지명    kr.co.devworld.zenkoi     ← **지금 이 화면에서 고정됩니다. 이후 변경·삭제·재사용 불가**
기본 언어   한국어 / English          ← 골라 주세요
앱 유형     게임
가격        무료                      ← **무료로 내면 이 패키지명으로는 유료 전환 불가**
→  이대로 만들까요?
```

**두 줄에만 경고를 단다.** 나머지는 나중에 고칠 수 있고(§4), 전부에 경고를 달면 사람이 둘을 구별해
읽지 못한다.

"나중에 확정된다" 고 읽힐 여지를 남기지 않는다 — 사람이 "아직 되돌릴 수 있겠지" 로 읽고 지나가는
것이 이 화면이 막으려는 것이다.
...
```
HANDOFF_REQUIRED=store_session_missing
...
- **무료/유료 결정** — 무료는 그 패키지명에 대해 되돌릴 수 없다(§4)
