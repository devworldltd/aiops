...
## 산출물 검증 규약 (모든 STEP 공통, 생략 금지)

**등록은 완료가 아니다. 되읽어 대조해야 완료다.**

한 번의 devflow 에서 서로 다른 에이전트 7명이 실패를 성공으로 보고한 적이 있다. 넷은 `forge.sh` 를 잘못 호출해 **플래그 문자열이 본문으로 등록**됐는데 정상 `COMMENT_ID` 를 받아 "등록 완료" 로 보고했고, 셋은 **확인하지 않은 추정을 사실처럼** 적었다(`IN ()` 가 500 을 낸다고 썼으나 실측은 200). 마커 계약이 조용히 깨진 채 다음 STEP 이 진행됐다.

도구는 고쳤지만(`forge.sh` 가 이제 옵션 같은 위치 인자와 모르는 플래그를 거부한다) **두 번째 유형은 도구로 막을 수 없다.** 사람은 결과를 눈으로 보지만 에이전트는 반환값으로 판단한다 — 반환값이 그럴듯하면 거기서 멈춘다.

### 1. 등록한 산출물은 되읽어 첫 줄을 대조한다

```bash
forge.sh issue-comment <ISSUE_NUMBER> "## 📝 PRD
...본문..."
forge.sh issue-comments <ISSUE_NUMBER>   # 첫 줄이 기대 헤더와 일치하는지 확인한다
```

확인할 것은 **마커 헤더가 댓글의 첫 줄인지**와 **본문 길이가 산출물에 걸맞은지** 두 가지다. `COMMENT_ID` 를 받은 것은 확인이 아니다.

### 2. 오케스트레이터는 다음 STEP 진입 전 직전 마커를 검증한다

직전 STEP 의 마커가 **어느 댓글의 첫 줄로** 존재하는지 확인한다. 없으면 다음 STEP 으로 넘어가지 않고 해당 에이전트를 다시 호출한다.

> ⚠️ **전체 댓글을 이어붙여 검색하면 안 된다.** 다른 에이전트가 보고서 본문에 헤더를 인용하면 **없는데 있다고 판정된다** — 실제로 겪은 오탐이다. 반드시 댓글 단위로 첫 줄을 본다.

### 3. 보고에는 관찰한 사실을 적는다

`등록 완료 (ID=31864)` 가 아니라 `등록 후 issue-comments 로 재조회 → 첫 줄 "## 🧪 E2E 시나리오", 본문 750자 확인` 처럼 **무엇을 보고 그렇게 판단했는지**를 쓴다.

검증하지 않은 것은 추정이라고 표시한다. `400 을 반환한다` 가 아니라 `400 일 것으로 보인다(미실측)` 로 적는다. 실측과 추정을 섞으면 뒤 단계가 추정을 사실로 받아 쓴다.
